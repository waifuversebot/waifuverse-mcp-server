...
"""
        ...
        # Check if first message is system message, if not add mochi system message at front
        if rawTranscript[0].get('role') != 'system':
            rawTranscript.insert(0, MOCHI_SYSTEM_MESSAGE)
            logger.info("Inserted mochi system message at front of transcript")
        else:
            # Replace existing system message with mochi's system message
            logger.warning(f"!!! Replacing existing system message. Old: {rawTranscript[0].get('content', '')[:100]}...")
            rawTranscript[0] = MOCHI_SYSTEM_MESSAGE
        ...
        transcript.append({'role': 'system', 'content': MOCHI_SYSTEM_MESSAGE['content']})
        logger.info(f"  [sys] Injected mochi system message ({len(MOCHI_SYSTEM_MESSAGE['content'])} chars)")
